...
fn export_draw_file(map: &DrawMethod, path: &PathBuf) -> Result<()> {
	...
	for key in map.keys() {
		list.push(read_file_to_string(
			&format!("src/transpiler/template/draw_impl/{key}.template")
		)?);
	}
...
pub fn export_main_file(path: &PathBuf, width: u32, height: u32) -> Result<()> {
...
pub fn export_cargo_file(path: &PathBuf) -> Result<()> {
